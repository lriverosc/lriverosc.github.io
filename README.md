...
- **TON 618**: easter egg ASCII en la esquina superior derecha del fondo espacial, por encima de donde ronda el teclado 3D — el arte (`src/data/ton618-ascii.ts`) resuelve lentamente desde la oscuridad, se mantiene nítido unos segundos y vuelve a disolverse, en un ciclo de ~20s. Es CSS puro (opacity + blur + transform, sin imágenes ni Canvas/WebGL — ver [`src/components/black-hole.tsx`](src/components/black-hole.tsx)); bajo `prefers-reduced-motion` se muestra estático y nítido en vez de ocultarse.
