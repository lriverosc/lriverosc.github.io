...
- **Presentación general** en el Hero (no acotada a un solo rubro) con un eslogan propio.
- **TON 618**: easter egg en el fondo espacial — la foto real del agujero negro se funde con el fondo cada cierto tiempo en la esquina superior derecha, por encima de donde ronda el teclado 3D, con el plasma naranja animado mediante un shader WebGL que distorsiona solo las zonas cálidas de la textura (el horizonte de sucesos y el fondo quedan fijos) y una máscara radial que disuelve los bordes rectangulares de la foto en vez de mostrarla como un recuadro ([`src/components/black-hole.tsx`](src/components/black-hole.tsx), imagen en `public/assets/luis/ton618.png`). Se desactiva junto con el resto de efectos decorativos si el dispositivo pide `prefers-reduced-motion` o es de gama baja.
