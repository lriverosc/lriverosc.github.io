...
- **TON 618**: la imagen real del agujero negro vive junto al texto del Hero, con el plasma naranja animado mediante un shader WebGL que distorsiona solo las zonas cálidas de la textura — el horizonte de sucesos y el fondo quedan fijos ([`src/components/black-hole.tsx`](src/components/black-hole.tsx), imagen en `public/assets/luis/ton618.png`). Se desactiva junto con el resto de efectos decorativos si el dispositivo pide `prefers-reduced-motion` o es de gama baja.
