...
import keyboardScene from "@/data/keyboard-scene.json";
...
const KeyboardScene = ({ maxDpr }: { maxDpr: number }) => {
  ...
  const scanAnimationRef = useRef<{ start: () => void; stop: () => void }>(null);
  ...
  const getScanAnimation = () => {
    const keys = Object.values(SKILLS)
      .map(skill => splineApp?.findObjectByName(skill.name))
      .filter((key): key is SPEObject => Boolean(key));
    // A restrained traveling pulse suggests a system processing data.
    // Animate whole keys, leaving their individual press events intact.
    const resting = keys.map(key => key.position.y);
    let timeline: gsap.core.Timeline | undefined;
    const stop = () => {
      timeline?.kill();
      timeline = undefined;
      keys.forEach((key, index) => { key.position.y = resting[index]; });
    };
    const start = () => {
      stop();
      timeline = gsap.timeline({ repeat: -1, repeatDelay: 1.8 });
      keys.forEach((key, index) => {
        timeline!.to(key.position, {
          y: resting[index] + 22,
          duration: 0.4,
          ease: "sine.inOut",
          yoyo: true,
          repeat: 1,
        }, (index % 6) * 0.14 + Math.floor(index / 6) * 0.1);
      });
    };
    return { start, stop };
  };
  ...
  useEffect(() => {
    ...
    const manageAnimations = async () => {
      ...
      // Handle the traveling processing pulse
  ...
  // processing pulse, which are only visible through it) while the tab is
