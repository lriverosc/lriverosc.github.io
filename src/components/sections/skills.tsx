...
const SkillsSection = () => {
    ...
    return (
      ...
      >
        <SectionHeader
          id="skills"
          title="Tecnologías"
          desc="Herramientas con las que construyo soluciones."
        ...
        <ul className="mx-auto grid w-full max-w-5xl grid-cols-2 gap-3 px-4 sm:grid-cols-3 sm:gap-4 md:grid-cols-4 lg:grid-cols-5">
          {Object.values(SKILLS).map((skill) => (
            ...
            >
              ...
              <img
                src={`${skill.icon}?v=neuralcore-symbols-3`}
                alt={skill.label}
                width={44}
                height={44}
                loading="lazy"
  ...
  return (
    ...
    >
      <ul className="sr-only">{Object.values(SKILLS).map(skill => <li key={skill.name}>{skill.label}: {skill.shortDescription}</li>)}</ul>
      <SectionHeader id="skills" title="Tecnologías" desc="Explora el teclado: cada tecla representa una de mis tecnologías." />
    </SectionWrapper>
  );
};
